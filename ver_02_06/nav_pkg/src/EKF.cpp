...
int main(int argc, char **argv)
{
	...
	ROS_WARN("Initialization"); //da rivedere!
	while (isInit == 0) 
	{
		loop_rate.sleep();
		ros::spinOnce();
	...
	while(ros::ok())
	{
		...
		if(usbl_obj.get_isNew())
		{
			...
			//Nota: Questa divisione e' provvisoria, perche' ho cambiato 
			//la sensor_function dell'usbl e mi secca correggerla
			...
			if(mahalanobis(e, H*P*H.transpose() + M*R*M.transpose()) && (n_eff ==2))
			{
