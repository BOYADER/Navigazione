...
x3_upd=(eye(3) + T*skew_product(J2*u2 +w_gyro))*x3+ T*J1*w_acc;

state_upd=[x1_upd; x2_upd; x3_upd];

F=subs(diff_matrix_by_vector(state_upd, state), w_imu, zeros(6, 1));
D=subs(diff_matrix_by_vector(state_upd, w_imu), w_imu, zeros(6, 1));
